...
public class RestExceptionHandler {
  ...
  public ResponseEntity<BaseResponse<ExceptionResponse>> handleUserException(UserException exception) {
    return buildErrorResponse(exception.getErrorCode(), exception.getMessage(), HttpStatus.BAD_REQUEST);
  }

  @ExceptionHandler(MethodArgumentNotValidException.class)
  @ResponseStatus(HttpStatus.BAD_REQUEST)
  public ResponseEntity<BaseResponse<List<ExceptionResponse>>> handleValidationException(MethodArgumentNotValidException ex) {
    List<ExceptionResponse> errors = ex.getBindingResult().getFieldErrors().stream()
  ...
  @ResponseStatus(HttpStatus.BAD_REQUEST)
  public ResponseEntity<BaseResponse<ExceptionResponse>> handleIllegalArgumentException(IllegalArgumentException ex) {
  ...
  public ResponseEntity<BaseResponse<ExceptionResponse>> handleGenericException(Exception ex) {
    return buildErrorResponse("SERVER_ERROR", "An unexpected error occurred", HttpStatus.INTERNAL_SERVER_ERROR);
  }
